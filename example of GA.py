...
Original file is located at
    https://colab.research.google.com/drive/17onq0rHdcFXMAjrT0V8mQ0lpkZxX4-mg
"""
...
# Preprocess movie overviews and prompts
processed_overviews = [preprocess(overview) for overview in movie_overviews]
processed_prompts = [preprocess(prompt) for prompt in generated_prompts]

# Using TF-IDF to calculate text similarity
vectorizer = TfidfVectorizer()
tfidf_matrix = vectorizer.fit_transform(processed_overviews + processed_prompts)

# Calculate similarity between movie overviews and prompts
similarity_matrix = cosine_similarity(tfidf_matrix[:len(movie_overviews)], tfidf_matrix[len(movie_overviews):])

# Print similarity results
for i, overview in enumerate(movie_overviews):
    for j, prompt in enumerate(generated_prompts):
        print(f"Similarity - Overview: '{overview}' / Prompt: '{prompt}' = {similarity_matrix[i][j]}")
